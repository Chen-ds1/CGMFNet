...
def parse_args():
    parser = argparse.ArgumentParser(description='Train PSPNet')
...
class SegmentationLoss(nn.Module):
    ...
    def forward(self, main_output, target, auxiliary_output=None):
        ...
        if auxiliary_output is not None:
            auxiliary_loss = self.cross_entropy(auxiliary_output, target)
            main_loss = main_loss + self.aux_weight * auxiliary_loss
        return main_loss
    ...
    for step, (images, masks) in enumerate(progress, start=1):
        ...
        with autocast(enabled=amp_enabled):
            ...
            if isinstance(outputs, tuple):
                loss = criterion(outputs[0], masks, outputs[1])
            else:
